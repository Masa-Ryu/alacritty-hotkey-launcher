use crate::common_backend::{AppConfig, WaylandHideMethod, WindowBackend};
use crate::x11_ewmh::{matches_app, select_preferred_window, Candidate};
...
impl WaylandBackend {
    pub fn new() -> Self {
        Self {
            ...
            flavor: None,
            hide_method: WaylandHideMethod::Auto,
        }
    }
    ...
    pub fn new_with_config(cfg: &AppConfig) -> Self {
        Self {
            runner: Box::new(SystemRunner),
            flavor: None,
            hide_method: cfg.wayland_hide_method,
        }
    }

    // For tests: inject a fake runner
    #[cfg(test)]
    fn with_runner(r: Box<dyn Runner>) -> Self {
        Self {
            runner: r,
            flavor: None,
            hide_method: WaylandHideMethod::Auto,
    ...
    #[cfg(test)]
    fn with_runner_and_method(r: Box<dyn Runner>, m: WaylandHideMethod) -> Self {
        Self {
            runner: r,
            flavor: None,
            hide_method: m,
    ...
    fn sway_traverse<'a, F: FnMut(&'a SwayNode, Option<&'a str>)>(
        node: &'a SwayNode,
        ws: Option<&'a str>,
        f: &mut F,
    ) {
        ...
        f(node, this_ws);
        for n in &node.nodes {
            Self::sway_traverse(n, this_ws, f);
        }
    ...
    fn sway_focus_or_show(&mut self, id: u64) {
        ...
        let _ = self.runner.quiet(
            "swaymsg",
            &[&format!("[con_id={id}]"), "scratchpad", "show"],
        );
        let _ = self
            .runner
            .quiet("swaymsg", &[&format!("[con_id={id}]"), "focus"]);
    }

    fn sway_hide_to_scratchpad(&mut self, id: u64) {
        let _ = self.runner.quiet(
            "swaymsg",
            &[&format!("[con_id={id}]"), "move", "to", "scratchpad"],
        );
    }
    ...
    fn hypr_move_to_current_ws(&mut self, id: u64) {
        ...
        let _ = self
            .runner
            .quiet("hyprctl", &["dispatch", "focuswindow", &addr]);
        let _ = self
            .runner
            .quiet("hyprctl", &["dispatch", "movetoworkspace", "current"]);
    }
    ...
    fn hypr_hide_to_special(&mut self, id: u64) {
        ...
        let _ = self
            .runner
            .quiet("hyprctl", &["dispatch", "movetoworkspace", "special"]);
    }
}
...
impl WindowBackend for WaylandBackend {
    ...
    fn is_on_current_workspace(&mut self, window: u64) -> bool {
        match self.ensure_flavor() {
            Some(WlFlavor::Sway) => {
                let current = match self.sway_current_workspace() {
                    Some(s) => s,
                    None => return false,
                };
                let out = match self.runner.output("swaymsg", &["-t", "get_tree"]) {
                    Ok(s) => s,
                    Err(_) => return false,
                };
                let root: SwayNode = match serde_json::from_str(&out) {
                    Ok(r) => r,
                    Err(_) => return false,
                };
                let mut on_ws = false;
                Self::sway_traverse(&root, None, &mut |n, ws| {
                    if n.id == window {
                        on_ws = ws == Some(current.as_str());
                    }
                });
                on_ws
            }
            Some(WlFlavor::Hyprland) => {
                let current = match self.hypr_current_workspace_id() {
                    Some(id) => id,
                    None => return false,
                };
                let out = match self.runner.output("hyprctl", &["-j", "clients"]) {
                    Ok(s) => s,
                    Err(_) => return false,
                };
                let clients: Vec<HyprClient> = match serde_json::from_str(&out) {
                    Ok(v) => v,
                    Err(_) => return false,
                };
                clients.into_iter().any(|c| {
                    hypr_parse_address(&c.address) == Some(window) && c.workspace.id == current
                })
            }
            Some(WlFlavor::Gnome) => {
                let current = match self.gnome_current_workspace() {
                    Some(id) => id,
                    None => return false,
                };
                let wins = match self.gnome_list_windows() {
                    Some(v) => v,
                    None => return false,
                };
    ...
    fn is_visible(&mut self, window: u64) -> bool {
        match self.ensure_flavor() {
                ...
                Self::sway_traverse(&root, None, &mut |n, ws| {
                    if n.id == window {
                        if let Some(ws_name) = ws {
                            visible = ws_name == current && ws_name != "__i3_scratch";
                        }
                    }
                });
                visible
            ...
            Some(WlFlavor::Gnome) => {
                ...
                wins.into_iter()
                    .any(|w| w.id == window && w.ws == current && !w.minimized)
            }
            None => false,
        }
    }
    ...
    fn move_to_current_workspace(&mut self, window: u64) {
        match self.ensure_flavor() {
            Some(WlFlavor::Sway) => {
                self.sway_move_to_current_ws(window);
            }
            Some(WlFlavor::Hyprland) => {
                self.hypr_move_to_current_ws(window);
            }
            Some(WlFlavor::Gnome) => {
                self.gnome_move_to_current_ws(window);
            }
            None => {}
        }
    }
    ...
    fn show(&mut self, window: u64) {
        match self.ensure_flavor() {
            Some(WlFlavor::Sway) => {
                self.sway_focus_or_show(window);
            }
            Some(WlFlavor::Hyprland) => {
                self.hypr_focus_or_show(window);
            }
            Some(WlFlavor::Gnome) => {
                self.gnome_focus_or_show(window);
            }
            None => {}
        }
    }
    ...
    fn hide(&mut self, window: u64) {
        match (self.ensure_flavor(), self.hide_method) {
            (Some(WlFlavor::Sway), WaylandHideMethod::Scratchpad | WaylandHideMethod::Auto) => {
                self.sway_hide_to_scratchpad(window)
            }
...
struct HyprMonitor {
    ...
    #[allow(dead_code)]
    id: i64,
    activeWorkspace: HyprActiveWorkspace,
}
...
impl WaylandBackend {
    ...
    fn gnome_collect_candidates(&mut self, target: &str) -> io::Result<Vec<Candidate>> {
        let wins = self
            .gnome_list_windows()
            .ok_or_else(|| io::Error::new(io::ErrorKind::Other, "gnome list windows failed"))?;
...
mod tests {
    ...
    struct FakeRunner {
        ...
        gnome_windows: String,
        gnome_ws: String,
        invoked: Vec<(String, Vec<String>)>,
    }

    impl FakeRunner {
        fn new(tree_json: &str, ws_json: &str) -> Self {
            Self {
                tree_json: tree_json.to_string(),
                ws_json: ws_json.to_string(),
                hypr_clients: String::new(),
                hypr_monitors: String::new(),
                gnome_windows: String::new(),
                gnome_ws: String::new(),
                invoked: Vec::new(),
            }
        }
        fn with_hypr(clients: &str, monitors: &str) -> Self {
            Self {
                tree_json: String::new(),
                ws_json: String::new(),
                hypr_clients: clients.to_string(),
                hypr_monitors: monitors.to_string(),
                gnome_windows: String::new(),
                gnome_ws: String::new(),
                invoked: Vec::new(),
            }
        }
        fn with_gnome(windows: &str, ws: &str) -> Self {
            Self {
                tree_json: String::new(),
                ws_json: String::new(),
                hypr_clients: String::new(),
                hypr_monitors: String::new(),
                gnome_windows: windows.to_string(),
                gnome_ws: ws.to_string(),
                invoked: Vec::new(),
            }
        }
    }
    ...
    impl Runner for FakeRunner {
        fn output(&mut self, program: &str, args: &[&str]) -> io::Result<String> {
            ...
            } else if program == "gdbus"
                && args.len() >= 9
                && args[0] == "call"
                && args[3] == "org.gnome.Shell"
                && args[5] == "/org/gnome/Shell"
                && args[7] == "org.gnome.Shell.Eval"
            {
    ...
    fn gnome_ws_json() -> String {
        r#"{"ws":1}"#.to_string()
    }

    #[test]
    fn sway_find_prefers_current_ws_visible() {
        let runner = Box::new(FakeRunner::new(
            &sample_tree(),
            &sample_workspaces_current1(),
        ));
    ...
    fn sway_commands_for_move_show_hide() {
        ...
        let fr = be
            .runner
            .as_any()
            .downcast_ref::<FakeRunner>()
            .expect("expected FakeRunner");
        let cmds: Vec<String> = fr
            .invoked
            .iter()
        ...
        assert!(cmds
            .iter()
            .any(|c| c.contains("swaymsg [con_id=101] move to workspace current")));
        assert!(cmds
            .iter()
            .any(|c| c.contains("swaymsg [con_id=101] scratchpad show")));
        assert!(cmds
            .iter()
            .any(|c| c.contains("swaymsg [con_id=101] focus")));
        assert!(cmds
            .iter()
            .any(|c| c.contains("swaymsg [con_id=101] move to scratchpad")));
    }
    ...
    fn hypr_find_and_visibility() {
        let runner = Box::new(FakeRunner::with_hypr(
            &hypr_clients_json(),
            &hypr_monitors_json(),
        ));
        let mut be = WaylandBackend::with_runner(runner);
    ...
    fn hypr_commands_for_move_show_hide() {
        ...
        assert!(cmds
            .iter()
            .any(|c| c.contains("hyprctl dispatch focuswindow address:0x101")));
        assert!(cmds
            .iter()
            .any(|c| c.contains("hyprctl dispatch movetoworkspace current")));
        assert!(cmds
            .iter()
            .any(|c| c.contains("hyprctl dispatch movetoworkspace special")));
    }

    #[test]
    fn gnome_find_visibility_and_commands() {
        let runner = Box::new(FakeRunner::with_gnome(
            &gnome_windows_json(),
            &gnome_ws_json(),
        ));
        let mut be = WaylandBackend::with_runner(runner);
